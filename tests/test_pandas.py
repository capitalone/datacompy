...
def test_compare_df_setter_good():
    ...
    compare = PandasCompare(df1, df2, ["A", "b"])
    assert compare.df1.equals(df1)
    assert compare.df2.equals(df2)
...
def test_compare_df_setter_bad_index():
    df = pd.DataFrame([{"a": 1, "A": 2}, {"a": 2, "A": 2}])
    with raises(TypeError, match="df1 must be a pandas DataFrame"):
        compare = PandasCompare("a", "a", on_index=True)
    with raises(ValueError, match="df1 must have unique column names"):
        compare = PandasCompare(df, df.copy(), on_index=True)
...
def test_compare_on_index_and_join_columns():
    ...
    with raises(Exception, match="Only provide on_index or join_columns"):
        compare = PandasCompare(df, df.copy(), on_index=True, join_columns=["a"])
...
def test_columns_maintain_order_through_set_operations():
    ...
    compare = PandasCompare(df1, df2, ["join"])
...
def test_10k_rows():
    ...
    df2 = df1.copy()
    df2["b"] = df2["b"] + 0.1
    compare_tol = PandasCompare(df1, df2, ["a"], abs_tol=0.2)
    ...
    compare_no_tol = PandasCompare(df1, df2, ["a"])
...
def test_string_joiner():
    ...
    compare = PandasCompare(df1, df2, "ab")
...
def test_simple_dupes_two_fields():
    ...
    compare = PandasCompare(df1, df2, join_columns=["a", "b"])
    assert compare.matches()
    # Just render the report to make sure it renders.
    t = compare.report()
...
200,0,2017-07-01,1009393,2.01,2017-06-29,D,USA,5814,22102,,F,"""
    ...
    df2 = df1.copy()
    compare_acct = PandasCompare(df1, df2, join_columns=["acct_id"])
    assert compare_acct.matches()
    compare_unq = PandasCompare(
        df1,
        df2,
...
def test_strings_with_joins_with_ignore_spaces():
    ...
    compare = PandasCompare(df1, df2, "a", ignore_spaces=False)
    assert not compare.matches()
    assert compare.all_columns_match()
    assert compare.all_rows_overlap()
    assert not compare.intersect_rows_match()

    compare = PandasCompare(df1, df2, "a", ignore_spaces=True)
    assert compare.matches()
    assert compare.all_columns_match()
    assert compare.all_rows_overlap()
    assert compare.intersect_rows_match()
...
def test_strings_with_joins_with_ignore_case():
    df1 = pd.DataFrame([{"a": "hi", "b": "a"}, {"a": "bye", "b": "A"}])
    df2 = pd.DataFrame([{"a": "hi", "b": "A"}, {"a": "bye", "b": "a"}])
    compare = PandasCompare(df1, df2, "a", ignore_case=False)
    assert not compare.matches()
    assert compare.all_columns_match()
    assert compare.all_rows_overlap()
    assert not compare.intersect_rows_match()

    compare = PandasCompare(df1, df2, "a", ignore_case=True)
    assert compare.matches()
    assert compare.all_columns_match()
    assert compare.all_rows_overlap()
    assert compare.intersect_rows_match()
...
def test_index_with_joins_with_ignore_spaces():
    df1 = pd.DataFrame([{"a": 1, "b": " A"}, {"a": 2, "b": "A"}])
    df2 = pd.DataFrame([{"a": 1, "b": "A"}, {"a": 2, "b": "A "}])
    compare = PandasCompare(df1, df2, on_index=True, ignore_spaces=False)
    assert not compare.matches()
    assert compare.all_columns_match()
    assert compare.all_rows_overlap()
    assert not compare.intersect_rows_match()
...
def test_index_with_joins_with_ignore_case():
    df1 = pd.DataFrame([{"a": 1, "b": "a"}, {"a": 2, "b": "A"}])
    df2 = pd.DataFrame([{"a": 1, "b": "A"}, {"a": 2, "b": "a"}])
    compare = PandasCompare(df1, df2, on_index=True, ignore_case=False)
    assert not compare.matches()
    assert compare.all_columns_match()
    assert compare.all_rows_overlap()
    assert not compare.intersect_rows_match()
...
def test_sample_mismatch():
    ...
    compare = PandasCompare(df1, df2, "acct_id")
...
def test_dupes_with_nulls():
    ...
    comp = PandasCompare(df1, df2, join_columns=["fld_1", "fld_2"])
    assert comp.subset()
...
def test_lower():
    ...
    df1 = pd.DataFrame({"a": [1, 2, 3], "b": [0, 1, 2]})
    df2 = pd.DataFrame({"a": [1, 2, 3], "B": [0, 1, 2]})
    compare = PandasCompare(df1, df2, join_columns=["a"], cast_column_names_lower=False)
    assert not compare.matches()
...
def test_save_html(mock_render):
    ...
    m = mock.mock_open()
    with mock.patch("datacompy.pandas.open", m, create=True):
